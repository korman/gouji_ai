...
class StrategySystem(esper.Processor):
    ...
    def select_strategy(self, player_id: int, strategy: PlayStrategy) -> List[Card]:
        ...
        if allow_split:
            ...
            all_combinations = CardPatternChecker.find_all_beating_combinations_without_splitting(
                hand_cards.cards, last_played_cards
            )
        ...
        # 根据不同策略选择不同牌组
        if strategy == PlayStrategy.PLAY_MINIMAL or strategy == PlayStrategy.PLAY_MINIMAL_INTACT:
            # 选择最小的牌组合
            return min(all_combinations, key=lambda cards: sum(card.rank.get_value() for card in cards))
            ...
            return min(multis, key=lambda cards: sum(card.rank.get_value() for card in cards))
